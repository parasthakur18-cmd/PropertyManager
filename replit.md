...
## Recent Updates
- **January 6, 2026**: Implemented separate login flows for Super Admin vs Regular Admin. Super Admin uses dedicated `/super-admin-login` URL with `/api/auth/super-admin-login` endpoint (only accepts super-admin role). Regular admins use `/login` with `/api/auth/email-login` (rejects super-admin accounts). This separation ensures security and prevents confusion between admin portals.
